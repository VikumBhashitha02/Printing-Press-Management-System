...
const Products = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((result) => {
        const fetchedProducts = result.data.existingProducts;
  ...
  const filteredProducts = products.filter(
    (item) =>
      ...
      (item.pcategory &&
        item.pcategory.toLowerCase().includes(searchQuery.toLowerCase())) // Ensure category name is not undefined or null
  );
          ...
          <tbody>
            {filteredProducts.map((product) => (
              <tr key={product._id}>
                ...
                <td>
                  <Link
                    to={`/admin/products/ProductDetails/${product._id}`}
                ...
                <td>
                  ...
                  {images[product._id] ? (
                    <img
                      ...
                      src={`data:image/jpeg;base64,${images[product._id]}`}
                      alt={product.pname}
